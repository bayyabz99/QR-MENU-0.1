...
// Şefin önerileri (promotions)
router.get('/promotions/chef-picks', (req, res) => {
  const db = dbModule.getDb();
  db.get('SELECT * FROM promotions WHERE section_name = ? AND is_active = 1', ['chef_picks'], (err, promotion) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    
    if (!promotion || !promotion.product_ids) {
      return res.json({ title_tr: 'Şefin Önerileri', title_en: 'Chef\'s Recommendations', products: [] });
    }
    
    // Product ID'leri parse et
    const productIds = promotion.product_ids.split(',').map(id => parseInt(id.trim())).filter(id => !isNaN(id));
    
    if (productIds.length === 0) {
      return res.json({ title_tr: promotion.title_tr || 'Şefin Önerileri', title_en: promotion.title_en || 'Chef\'s Recommendations', products: [] });
    }
    ...
    db.all(
      ...
      (err, products) => {
        ...
        res.json({
          title_tr: promotion.title_tr || 'Şefin Önerileri',
          title_en: promotion.title_en || 'Chef\'s Recommendations',
          products: products || []
        });
      }
    );
