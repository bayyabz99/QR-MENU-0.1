...
router.get('/promotions', authenticateToken, (req, res) => {
  try {
    const db = dbModule.getDb();
    db.get('SELECT * FROM promotions WHERE section_name = ?', ['chef_picks'], (err, row) => {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
      res.json(row || {});
...
router.post('/promotions', authenticateToken, (req, res) => {
  try {
    const db = dbModule.getDb();
    const { title_tr, title_en, product_ids, menu_url } = req.body;
    
    const updatePromises = [
      new Promise((resolve, reject) => {
        db.run('INSERT OR REPLACE INTO promotions (section_name, title_tr, title_en, product_ids, is_active) VALUES (?, ?, ?, ?, 1)',
          ['chef_picks', title_tr, title_en, product_ids || ''],
          (err) => {
            if (err) reject(err);
            else resolve();
          }
